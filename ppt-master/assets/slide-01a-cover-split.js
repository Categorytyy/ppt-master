// slide-01a-cover-split.js - 封面-左右分栏
...
function createSlide(pres, theme, options = {}) {
  ...
  if (options.rightContent === 'image' && options.imagePath) {
    // 实际使用时传入图片路径
    slide.addImage({ data: options.imagePath, x: 4.5, y: 0.8, w: 5, h: 4 });
  } else {
    // 默认装饰块
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 4.5, y: 1.2, w: 4.8, h: 3.2,
      fill: { color: theme.secondary, transparency: 15 }
    });
    slide.addShape(pres.shapes.RECTANGLE, {
      x: 5.0, y: 1.7, w: 3.8, h: 2.2,
      fill: { color: theme.light, transparency: 30 }
    });
    // 右侧装饰文字
    slide.addText("BRAND", {
      x: 4.5, y: 2.2, w: 4.8, h: 1.2,
      fontSize: 48,
      fontFace: "Arial",
